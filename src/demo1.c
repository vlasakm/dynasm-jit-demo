...
//
// Apart from the checks the below function is a copy of the one from Peter
// Cawley's DynASM tutorial, we just do more checking with asserts.
//
...
compile(u8 *program, size_t program_len)
{
	...
	while (instrptr < end) {
		...
		case OP_INPUT: {
			...
			// `qword` respectively). But we can also include this
			// these keywords when they are not necessary, if we
			// prefer that, as we do below by specifying "dword
			// read", even though it would have been implicit from
			// the use of `eax` as the destination.
		...
		case OP_DISCARD: {
			...
			// value. That is what `OP_DISCARD` really is.
	...
	// Here we could keep the same DASM state, call `dasm_setup` and co
	dasm_free(Dst);
	return code;
}
